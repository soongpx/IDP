        ...
        for frame in frames:
            if frame.is_motion_frame():
                if frame.get_profile().stream_type() == rs.stream.accel:
                    ...
                    self.roll += gyro_data.y * self.dt  # Roll calculation
                    if accel_data is not None:
                        ...
                        roll_acc = np.arctan2(accel_data.z, np.sqrt(accel_data.x ** 2 + accel_data.y ** 2))  # Roll calculation
                        ...
                        self.roll = self.beta * (self.roll + gyro_data.y * self.dt) + (1 - self.beta) * roll_acc  # Roll calculation
                        self.target_pitch = np.degrees(self.pitch)
                        self.target_yaw = np.degrees(self.yaw)
                        self.target_roll = np.degrees(self.roll)  # Roll assignment
        ...
        for frame in frames:
            if frame.is_motion_frame():
                if frame.get_profile().stream_type() == rs.stream.accel:
                    ...
                    self.roll1 += gyro_data.y * self.dt  # Roll calculation
                    if accel_data is not None:
                        pitch_acc = np.arctan2(accel_data.y, np.sqrt(accel_data.x ** 2 + accel_data.z ** 2))
                        yaw_acc = np.arctan2(accel_data.x, np.sqrt(accel_data.y ** 2 + accel_data.z ** 2))
                        roll_acc = np.arctan2(accel_data.z, np.sqrt(accel_data.x ** 2 + accel_data.y ** 2))  # Roll calculation
                        ...
                        self.roll1 = self.beta * (self.roll1 + gyro_data.y * self.dt) + (1 - self.beta) * roll_acc  # Roll calculation
                        self.current_pitch = np.degrees(self.pitch1)
                        self.current_yaw = np.degrees(self.yaw1)
                        self.current_roll = np.degrees(self.roll1)  # Roll assignment

    def update_direction(self):
        pitch_diff = self.current_pitch - self.target_pitch
        yaw_diff = self.current_yaw - self.target_yaw
        roll_diff = self.current_roll - self.target_roll  # Roll difference calculation
        print(pitch_diff)
        print(yaw_diff)
        print(roll_diff)
        ...
        self.roll_direction = int(np.sign(roll_diff)) if abs(roll_diff) > 2 else 0  # Roll direction calculation
